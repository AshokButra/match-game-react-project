...
import MatchGameItem from './components/MatchGameItem'
...
class App extends Component {
  ...
  tick = () => {
    ...
    if (timeInterval <= 1) {
      ...
      this.setState({timeInterval: 60, resultPage: true})
    }
  ...
  onDisplaySelect = id => {
    const {objImage, timeInterval} = this.state
    if (timeInterval >= 0) {
      const randInt = Math.floor(Math.random() * imagesList.length)
      const randImage = imagesList[randInt]
      const object = imagesList.filter(eachObj => eachObj.id === id)
      const objImageUrl = object[0].imageUrl

      if (objImage === objImageUrl) {
        this.setState(prevState => ({
          score: prevState.score + 1,
          objImage: randImage.imageUrl,
          resultPage: false,
        }))
      } else {
        clearInterval(this.timerID)
  ...
  render() {
    const {objImage, activeTabId, timeInterval, score, resultPage} = this.state
            ...
            <li className="score-time-container">
              ...
              <div className="timer-count-container">
                <img
                  ...
                  alt="timer"
                />
                <p className="timer">{timeInterval} Sec</p>
              </div>
            </li>
          </ul>
          {resultPage ? (
            ...
            <>
              <img src={objImage} className="image-item" alt="match" />
              <ul className="tabs-container">
                {tabsList.map(eachItem => (
                  <li className="tab-list-item" key={eachItem.tabId}>
                    {activeTabId === eachItem.tabId ? (
                      <button
                        type="button"
                        className="button-tabs-active"
                        onClick={() => this.onClickButton(eachItem.tabId)}
                      >
              ...
              <ul className="list-container">
                {filteredList.map(eachItem => (
                  <MatchGameItem
                    ...
                    onDisplaySelect={this.onDisplaySelect}
                    key={eachItem.id}
                  />
                ))}
              </ul>
